...
### 色貌
<a id="色貌"></a>
**色貌 (color appearance)** 是指在特定的观测条件下，人眼感知到的色貌。色貌不仅依赖于物体本身的光谱特性，还受到观测条件的影响，如光源、背景颜色以及观察者的视觉状态。即使物体的物理颜色保持不变，观测条件的不同也可能导致色貌的变化。

### 色貌模型
<a id="色貌模型"></a>
**色貌模型 (color appearance model)** 用于解释颜色在不同观测条件下的感知变化。它帮助我们理解在不同的光照、背景和视觉适应状态下，色貌是如何改变的。这些模型对于色彩管理、图像处理以及视觉科学研究至关重要。

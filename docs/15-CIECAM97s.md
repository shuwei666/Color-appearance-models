...
7. 模型应能预测色相（包括色相角度和色相四分象限）、亮度、明度、饱和度、色度和色彩感知度。
8. 模型应能反向操作。
9. 模型不应比满足上述要求所需的复杂。
...
CIECAM97s 模型几乎可以被解析逆转，但由于在逆转过程中 Y 值的计算较为复杂（步骤 8），因此需要做出一个近似。从明度 $J$、色度 $C$ 和色相角度 $h$ 开始，逆转过程如下所示：
...
虽然 CIECAM97s 是一个相对简单的模型，但它在预测现象的种类上非常完整。它包括了所有重要的外观维度的相关因素（如明度、亮度、色度、色彩感知度、饱和度和色相），并且能够预测广泛的适应性、周围环境和亮度依赖性效应。它不适用于极高或极低的亮度水平，因为这些情况通常不涉及精确的颜色判断。表 15.2 中给出了使用 CIECAM97s 色貌模型进行的四个样本的示例计算，具体如本章所述。有关这些示例计算的电子表格，可以在 http://www.cis.rit.edu/ fairchild/CAM.html 网站找到。
...
ZLAB 模型在逆向操作时非常简单。从明度、色度和色相角度开始，按照以下步骤进行计算：

...
在接下来的几章中，将详细介绍历史上和目前被广泛研究和使用的色貌模型。本章重点讲述由 Nayatani 及其团队开发的色貌模型，这个模型是早期基于色度学的重要模型之一。和 Hunt 模型（详见第12章）一样，Nayatani 模型是一种完整的色貌模型，能够在多种观测条件下，预测第4章中提到的各种色貌参数。
...
3. **与 Hunt 模型的对比**  
   虽然两者都是完整的色貌模型，但 Nayatani 模型更强调“色适应”——也就是颜色如何随着光线和环境的变化而改变——在观测条件变化时尤其有效。
...
Nayatani 色貌模型是他们在第9章提到的“色适应模型”的自然延伸（Nayatani et al., 1981），并结合了 Hunt 在1982年和1985年首次提出的色貌模型概念（Hunt, 1982, 1985）。这个模型最初在1986年和1987年的研究中被描述，并在1995年进行了修订和总结（Nayatani et al., 1995）。本章主要描述1995版的模型。
...
模型主要用于预测“简单色块”在“中性灰色背景”上的色貌。它的设计特点是：

- 适合分析简单的视觉刺激，而非复杂图像或动态背景的变化；

- 输出值能够与主要的色貌属性（如明度、亮度、鲜艳度、彩度和色相）相对应。
...
Nayatani 色貌模型需要一组关键参数作为输入，用于描述刺激物、背景和观测条件的光学与色度特性。具体包括以下内容：
...
**从 CIE 三刺激值到视锥细胞响应的转换**

模型通过色品坐标计算适应场的中间值，用于表征视锥细胞响应：
...
与所有色貌模型一样，Nayatani 色貌模型的第一阶段是进行色适应（Chromatic Adaptation）的转换。该适应模型是第9章所述非线性色适应模型的精炼版本（CIE 1994；Nayatani et al., 1981）。在模型的构建过程中，色适应模型被嵌入到其他公式中。为便于理解，本节将遵循 Nayatani et al. (1995) 的处理方式，并在适当的公式中指出相关的色适应特性。

---

### 适应场的视锥细胞响应计算

适应场的视锥细胞响应需要基于绝对亮度水平计算。这一计算依赖于之前的色品转换公式（公式 11.3 至 11.5）、光照水平 \( E_o \) 和适应背景的亮度因子 \( Y_o \)。计算公式如下：
...
基于适应场的视锥细胞响应（公式 11.6），分别计算短波长（B）、中波长（G）和长波长（R）视锥细胞的指数值。公式如下：
...
归一化亮度的附加指数使用与中波长和长波长视锥细胞相同的形式，公式如下：
...
上述计算提供了在色貌模型中实现非线性色适应模型所需的所有中间数据。这些数据将在后续公式中被逐步使用，以完整预测颜色的外观特性。

## 11.4 对立色维度

在 Nayatani 色貌模型中，视锥细胞响应被直接转换为表示经典视觉响应的对立色维度：一个无色通道和两个色彩通道。这些公式同时包含了非线性色适应模型的元素。
...
- 无色响应是长波视锥细胞响应 \( R \) 和中波视锥细胞响应 \( G \) 的加权总和，权重为 \( \frac{2}{3} \) 和 \( \frac{1}{3} \)，对应人眼视网膜中两种视锥细胞的相对分布比例。

- 噪声项 \( n \) 通常取值为 1.0。

- 每个视锥细胞响应先经过 von Kries 型变换归一化（以适应背景的视锥细胞响应为基准）。

- 对归一化结果取对数，模拟人类视觉系统中已知的非线性压缩。

- \( e(R) \) 和 \( e(G) \) 是前面计算的缩放系数，\( \beta \) 是色适应的指数调整因子。
...
- **红-绿通道 \( t \)**  
  ...
  b. 短波视锥细胞 \( B_o \) 的响应对长波（红）部分有轻微影响，用于解释光谱紫的感知和独特黄的预测。

- **黄-蓝通道 \( p \)**  
  a. 通过 \( R_o + G_o \) 的加权和产生黄色响应；
  b. 减去 \( B_o \) 的响应生成对立的蓝色。

- 这些公式使用了 Hunt 模型中提出的加权因子，分别反映了视网膜视锥细胞的分布特点。
...
通过这些公式，模型能够同时计算测试样品的明亮度 \( Br \) 和理想白的明亮度 \( Br_w \)。这些计算结果为进一步分析样品的色貌提供了基础。
...
- 需要额外的模型来考虑该效应，特别是在比较不同色相或彩度的刺激物时。
...
在 Nayatani 色貌模型中，饱和度是最直接的色彩属性，它是计算鲜艳度和彩度的基础。饱和度包括两个分量：红-绿分量 \( S_{RG} \) 和黄-蓝分量 \( S_{YB} \)。
...
通过以上公式，模型能够计算饱和度的两个主要分量以及整体饱和度，为色貌的其他属性（例如鲜艳度和彩度）的计算提供了基础。
...
与传统定义的彩度相比，这些公式通过引入非线性项（指数 0.7），更精确地描述了真实观测条件下彩度的感知变化，同时考虑了明度和饱和度的非线性关系。
...
在 Nayatani 色貌模型中，鲜艳度的计算基于 CIE 对色貌属性的定义。彩度被定义为样品的鲜艳度相对于理想白亮度的比值。因此，鲜艳度可以通过样品的彩度乘以理想白的亮度来计算。

测试样品的红-绿鲜艳度、黄-蓝鲜艳度以及总体鲜艳度的计算公式分别如下：
...
鲜艳度通过结合彩度和理想白亮度，能够更准确地描述在给定光源和背景条件下，颜色的整体感知强度。此归一化设计确保了不同测试条件下的计算结果具有一致性和可比性。

---

通过以上公式，Nayatani 色貌模型可以有效预测颜色的鲜艳度，为进一步的色貌分析提供了基础。
...
幸运的是，Nayatani 色貌模型可以解析地进行逆运算。Nayatani 等人在 1990 年的一篇论文中介绍了如何反转模型以匹配亮度–鲜艳度或明度–彩度。这些方法虽然基于略早的模型版本，但基本过程仍适用。
...
a **获取物理数据**  
   从测试样品和观测条件中获取必要的物理参数，例如亮度、色品坐标、背景光照等。
...
f **计算鲜艳度 \( M \)**  
   根据公式 11.29–11.31，从彩度和理想白亮度推导鲜艳度。

---

### 正向与逆向关系

在逆模型中，以上步骤可以按照相反的顺序逐步求解。例如，从感知属性（如鲜艳度 \( M \)、彩度 \( C \)、明度 \( P_L^* \)）开始，通过逆向推导，逐步计算出物理数据（如视锥细胞响应、亮度等）。
...
Nayatani 色貌模型能够预测因色适应和亮度水平变化而引起的色貌变化，主要包括以下现象：
...
**支持的条件**：
  ...
  - 模型在处理色适应和亮度变化时具有较高的准确性。

**限制**：

  - 模型不适用于背景颜色变化（同时对比效应）或环境相对亮度变化（如 Bartleson–Breneman 方程）的预测。

  - 模型不包含对不完全色适应或认知光源补偿（discounting-the-illuminant）机制的预测能力。

---

### 3. 不完全色适应的处理

- Nayatani（1997）提出了一种基于实验数据的方法，用于估计不完全色适应的水平。
...
- 表格中展示的 \( S \)、\( C \)、\( M \) 分别对应饱和度、彩度和鲜艳度的计算结果。
...
- **全面性**：模型涵盖了鲜艳度、彩度、明度、饱和度和色相等所有主要色貌属性。
...
**未包含视觉视杆细胞的贡献**：
   ...
   - 视觉视杆细胞的影响（如 Hunt 模型中包含的）未被纳入，而这是颜色感知的重要部分。

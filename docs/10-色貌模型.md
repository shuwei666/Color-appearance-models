色适应变换（见第 9 章讨论）在三刺激色度学基础上向预测色貌的方向迈出了重要一步。然而，它们依然有局限性，只能预测不同观看条件下的匹配颜色（即对应颜色）。单靠色适应变换，无法描述刺激物的实际色貌。要做到这一点，必须使用第 4 章中定义的外观参数——包括**绝对色貌属性**（亮度、色彩度和色相）以及**相对色貌属性**（明度、彩度、饱和度和色相）。这些术语用来描述刺激物的色貌，而色适应变换并不能提供这些感知属性的对应指标。这就是**色貌模型**的作用领域。

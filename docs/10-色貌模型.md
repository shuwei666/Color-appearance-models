色适应变换（见第 9 章讨论）在三刺激色度学基础上向预测色貌的方向迈出了重要一步。然而，它们依然有局限性，只能预测不同观看条件下的匹配颜色（即对应颜色）。单靠色适应变换，无法描述刺激物的实际色貌。要做到这一点，必须使用第 4 章中定义的外观参数——包括**绝对色貌属性**（亮度、色彩度和色相）以及**相对色貌属性**（明度、色度、饱和度和色相）。这些术语用来描述刺激物的色貌，而色适应变换并不能提供这些感知属性的对应指标。这就是**色貌模型**的作用领域。
...
CIE 技术委员会 1-34 负责评估各种色貌模型的性能，并推荐一个可以普遍使用的模型。因此，委员会首先定义了什么是色貌模型，以便能够参与测试（Fairchild 1995a）。TC1-34 达成的共识定义是：**色貌模型是能够预测至少明度、色度和色相等相对色貌属性的模型**。要做到这一点，模型必须至少包含某种形式的**色适应变换**。

如果一个模型还能够预测**亮度**、**色彩度**，或者其他亮度相关效应（如 Stevens 效应或 Hunt 效应），那它就必须更加复杂。

根据上述定义，一些相对简单的均匀颜色空间，如 CIE 1976 L\*a\*b\* 颜色空间（CIELAB）和 CIE 1976 L\*u\*v\* 颜色空间（CIELUV），也可以被视为色貌模型。这些颜色空间包含简单的色适应变换，并且能够预测明度、色度和色相。

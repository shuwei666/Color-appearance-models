...
在上述公式中，*C*和*a***、**b***有相同的单位。对于无色刺激，*C*的值为0.0（即没有彩度）。色相角 **$h_{ab}$** 以正度数表示，0°位于正a*轴，并以逆时针方向进行。
...
图10.3展示了一个完整的、经过采样的CIELAB色彩空间，沿着明度、彩度和色相角维度显示。虽然CIELAB没有官方定义饱和度，但一个合理的近似值可以通过将*C*ab除以**L***来计算。

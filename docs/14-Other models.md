...
**LLAB** 模型是 CIE 色度学的扩展，旨在进行色彩匹配、色彩外观规范和色差测量。它与第13章中的 **RLAB** 模型结构相似，但在预测各种效应方面有所不同。
...
1. **视锥反应计算**：从 CIE XYZ 到标准化视锥反应，RGB 按照以下公式计算：
   ...
   G &= \left( \frac{Y}{Y} \right) \\
   B &= \left( \frac{Z}{Y} \right)
   ...
   \end{bmatrix} \tag{14.26}
   $$

2. **转换为参考视锥反应**：然后，视锥反应根据参考光源进行调整，使用以下公式：

   $$
   R_r = D \left( \frac{R_{or}}{R_0} \right) + (1 - D) R \tag{14.27}
   $$

   $$
   G_r = D \left( \frac{G_{or}}{G_0} \right) + (1 - D) G \tag{14.28}
   $$

   $$
   B_r = D \left( \frac{B_{or}}{B_0} \right) + (1 - D) B^\beta \tag{14.29}
   $$

   $$
   \beta = \left( \frac{B_0}{B_{0r}} \right)^{0.0834} \tag{14.30}
   $$

   如果 B 反应值为负，使用方程 (14.31) 替换方程 (14.29)，以避免计算负数的平方根：

   $$
   B_r = - \left( D \left( \frac{B_{or}}{B_{0r}} \right) + (1 - D) B^\beta \right) \tag{14.31}
   $$
...
$$
r_L = 116 \left( \frac{Y}{L_f} \right)^{1/3} - 16 \tag{14.33}
$$

$$
b_L = \left( \frac{Y}{z} \right)^{1/2} \tag{14.34}
$$

$$
A_f = 500 \left( \frac{X}{Y} - \frac{95.05}{100} \right) \tag{14.35}
$$

$$
B_f = 200 \left( \frac{Y}{Z} - \frac{108.88}{100} \right) \tag{14.36}
$$

**z 指数**被用来考虑背景的明度对比度，其形式类似于 Hunt 模型中的使用方法。由于 **L**、**A** 和 **B** 维度遵循 CIELAB 公式的定义，因此非线性依赖于相对三刺激值，如下所示：

**非线性调整：**

当 \( w > 0.008856 \) 时，使用方程 14.37：

$$
\omega = \frac{1}{F_f} \tag{14.37}
$$

当 \( w \leq 0.008856 \) 时，使用方程 14.38：

$$
\omega = \left( \frac{16}{116} \right) \left( \frac{1}{F_f} - 0.008856 \right) \tag{14.38}
$$

**F_s**（环境相对亮度因子）的值依赖于环境的相对亮度，具体值见 **表 14.2**，这与 **RLAB** 模型中包含的环境依赖性相似。

...
RLAB被设计为仅包括相对色貌属性的预测因子。因此，它可以用来计算明度、色彩度、饱和度和色相的相关属性，但不能用于预测亮度或色彩感知度。这一限制的设定是为了使模型尽可能简洁，并且因为在大多数图像再现应用中，亮度和色彩感知度的预测并不重要。
...
RLAB模型仅提供相对色貌属性的相关参数（明度、彩度、饱和度和色相）。它不能用于预测亮度和色彩丰富度。对于图像再现应用而言，这一限制几乎没有实际意义，因为在大多数应用中，亮度和色彩丰富度并不需要。RLAB包括一个色适应变换，具有消光因子参数，并预测对某些刺激的未完全色适应（例如，RLAB正确预测了D50白点的CRT显示器会保留黄色外观）。它还包括可变指数s，这些指数调节图像对比度，作为背景相对亮度的函数。这些是跨媒体图像再现应用中最重要的色貌现象。

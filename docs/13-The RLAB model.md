...
本章完成了对一些最广泛使用的历史性（CIE模型之前）色貌模型的讨论，并介绍了RLAB模型。虽然前几章中讨论的Hunt模型和Nayatani等模型是为了预测广泛观看条件下色貌的所有感知属性，但RLAB模型则是基于其他考虑而设计的。RLAB模型的开发目的是创建一个简单的色貌模型，能够预测实际应用中最重要的外观现象。RLAB的目标应用是跨媒体图像再现，并且已在此类情境中得到了有效的应用。
...
RLAB的概念是利用CIELAB在日光下的良好间距和空间熟悉度，同时改善其对非日光光源的适用性。这是通过定义一组参考观看条件（光源D65，318 cd/m²，平均背景，消光）来实现的，在这些条件下使用CIELAB空间，并通过使用更准确的色适应变换（Fairchild 1991b, 1994b, 1996）来确定测试观看条件和参考观看条件之间的对应颜色。测试三刺激值首先被转换为参考观看条件下的对应颜色，然后使用修改后的CIELAB空间来描述外观相关的属性。这个概念可以应用于任何所需的色适应变换，如基于CIECAM02的CAT02变换。
...
**R矩阵**表示参考观看条件（M*A-1）矩阵的逆，并包括归一化处理，如下所述，它是恒定的，因此可以预先计算。方程13.13代表了修改后的von Kries色适应变换，能够将测试刺激的三刺激值转换为在RLAB参考观看条件下的对应颜色。
...
这是R矩阵的表示，它代表了逆M和A矩阵的运算，用于参考观看条件的逆变换。该矩阵用于色适应转换，可以将测试三刺激值转换为参考条件下的相应颜色。
...
**图 13.2** 展示了改变幂指数的效果。左图（a）是典型的打印图像。其他两幅图像（b 和 c）展示了如果图像在暗背景下观看，需要调整的对比度。请注意，在暗背景下观看时，需要增加对比度。在中间图像（b）中，仅对明度对比度进行了调整。在右图（c）中，背景补偿同时应用于明度和彩度。右图（c）类似于简单的摄影系统中产生的再现（因为所有三个胶片层的对比度必须一起变化），通常是更理想的。然而，现实胶片中的图像间效应可以用于补偿对比度变化带来的饱和度增加（数字系统中的矩阵操作也可以做到这一点）。图13.2中的图像应仅用于判断调整的相对影响，因为它们并未在适当的观看条件下查看。
